...
{
	String strLogFileNamePath = ExtractFilePath(Application->ExeName);
	try
	{
		memoLog->Lines->SaveToFile(strLogFileNamePath + stMyPId->Caption + "_" + strLogFileName);
		MessageDlg(stMyPId->Caption + "_" + strLogFileName + " saved", mtInformation, TMsgDlgButtons() << mbOK, 0);
	}
	catch(Exception& e)
	{
