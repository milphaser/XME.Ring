...
void __fastcall TdmChannels::ReadIniFile(void)
{
	...
	formMain->stPId->Caption = pidThis->tag;
	...
	for(int i = 1; true; i++)
	{
		...
		id = iniFile->ReadInteger(strSection, L"ID", 0);
		tag = iniFile->ReadString(strSection, L"Tag", PID_NULL);
		ep.first = iniFile->ReadString(strSection, L"IP", L"127.0.0.1");
		ep.second = iniFile->ReadInteger(strSection, L"Port", 0);

		ListPIds.push_back(PID<int>(id, tag, ep));
	}
	...
	ListPIds.pop_front();
	pidNext = std::unique_ptr<PID<int>>(new PID<int>(item.id, item.tag, item.ep));
	formMain->stPIdNext->Caption = pidNext->tag;

	csOut->Address = pidNext->ep.first;
	csOut->Port = pidNext->ep.second;
}
//---------------------------------------------------------------------------
...
void __fastcall TdmChannels::csOutConnect(TObject *Sender, TCustomWinSocket *Socket)
{
	...
	formMain->panelRingUp->Color = clRUPStatus[static_cast<int>(rupState)];
	String stringMsgOut = MSG_MRK_RING + MSG_SEPARATOR + pidThis->tag;
		  ...
		  TErrorEvent ErrorEvent, int &ErrorCode)
{
	...
	{
		int intCurrentScale = ListPIds.size();
		formMain->labelCurrentScale->Caption = intCurrentScale;

		if(intCurrentScale >= MIN_K)
		{
			auto item = ListPIds.front();
			ListPIds.pop_front();
			pidNext = std::unique_ptr<PID<int>>(new PID<int>(item.id, item.tag, item.ep));
...
void __fastcall TdmChannels::E_OnStart(void)
{
	...
	String stringMsgOut = MSG_ELECTION + MSG_SEPARATOR + pidThis->tag;
	csOut->Socket->SendText(stringMsgOut);
...
void __fastcall TdmChannels::E_OnReceiptOfElection(String stringMsg, TCustomWinSocket *Socket)
{
	...
	else if(stringMsgPId < pidThis->tag)
	{   // j < i
	...
	{   // j = i    I am the Coordinator
		strElectedPId = pidThis->tag;
		formMain->stCoordinator->Caption = strElectedPId;
		eState = E_State::NONPARTICIPANT;

		String stringMsgOut = MSG_ELECTED + MSG_SEPARATOR + pidThis->tag;
		csOut->Socket->SendText(stringMsgOut);
...
void __fastcall TdmChannels::ME_OnReceiptOfMarker(String stringMsg)
{
	...
	if(meState == ME_State::WANTED && IsTimeMrkMENotNull())
	{
		...
		// ENTRANCE INTO THE CRITICAL SECTION - access()
	}
